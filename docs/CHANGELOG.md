...
## Android 16 Docker runtime tooling — 2026-08-03

- Parameterizes one emulator Dockerfile and entrypoint for API 35 and Android 16/API 36 Google APIs x86_64 images while keeping `minSdk 24`, `compileSdk 35`, and `targetSdk 35`. Both requested emulator images built successfully from local `zeam-docker-dev:android35`; this is runtime tooling, not target-SDK-36 or edge-to-edge migration.
- Extends required APK wrapper with `--with-android-test`, preserving stable debug signing and building app plus instrumentation APKs in one pinned Docker invocation. Shared runner now verifies exact runtime API, runs full instrumentation by default, performs Launcher focus/workspace smoke, records API-qualified diagnostics, and rejects bounded app-relevant fatal, ANR, verifier/missing API, `UnsupportedOperationException`, and AppWidget service logcat markers.
- Extends `run-zmreborn` with isolated API 35/API 36 test runtimes, daemon-verified exact current source/entrypoint mounts, read-only source mount, artifact-only writable probes and diagnostics, AVD-bound stale-runtime rejection, Docker-daemon KVM preflight before APK build or emulator boot, overflow-safe host-port validation, loopback-only ADB/noVNC publication, container-internal noVNC readiness, and documented SSH tunneling. Hosted GitHub Actions remains explicit API 24 coverage; no hosted API 36 claim is added.
- RED/GREEN shell coverage includes API mismatch, diagnostics path escape, logcat gating, Android-test build mode, missing-container creation, artifact-only daemon write probes, distinct/loopback-only ports, integer-overflow rejection, AVD runtime identity, container-internal noVNC readiness, and remote Docker-daemon KVM validation. All shell contracts and changed-script syntax passed. Full JVM suite passed 210 tests with no failures, errors, or skips; lint, app plus Android-test wrapper build, default wrapper build, exact API 35/API 36 system-image checks, and `git diff --check` passed. Final app APK remains 957,330 bytes with SHA-256 `56817ef665661b0889581c9e6b4e89c3e27da3817e5adf8db3eae0a0d556e587`; Android-test APK is 213,262 bytes with SHA-256 `0c751cd0a0b5d7abb71114c9f344f37a16d4b14d962a27ea4377af93ea2906bc`.
- Fresh local native Docker/KVM runs completed the full selector-unset instrumentation suite on API 35 and Android 16/API 36 after reviewer hardening: each passed all 133 tests with `INSTRUMENTATION_CODE: -1`. Each run also passed exact Launcher focus/workspace smoke and the bounded filtered-logcat gate.
- The first API 36 full run exposed a test-fixture `appwidget --user current` grant issue. Its focused RED failed; the fixture now resolves `am get-current-user`, validates the numeric ID, captures that exact user-bound grant operation in a cleanup token, and revokes test-added authority at most once. Focused picker and insertion tests passed before both full suites.
- One fresh API 36 post-hardening attempt reported `PreferencesE2ETest.testReducingScreenCountClampsDefaultAndFlushesOnPause` as `expected:<2> but was:<3>`; that method immediately passed alone and the next selector-unset 133-test run passed. No production fix is claimed for that intermittent order/timing signal.
- These runs do not claim hands-on real third-party bind approval, configuration, move/delete, or resize validation, nor an actual SystemUI gesture.

## Unreleased widget insertion fix — 2026-08-03

- Fixes provider cards that consumed pointer activation without invoking the picker list listener, leaving the selection dialog open with no widget action. Card roots now leave activation to `ListView`, and Launcher dismisses and clears picker ownership before Search insertion, widget ID allocation, bind approval, or provider configuration.
- Adds real-pointer regression coverage using `ACTION_DOWN`/`ACTION_UP` through the production list. RC12 RED failed with `Provider touch must invoke picker selection callback`; the fixture-backed production-flow RED failed with `Provider touch did not dismiss production widget picker`.
- Adds instrumentation-APK-only external widget provider/configuration fixtures plus `WidgetInsertionE2ETest`. The E2E enters through the visible production Add dialog, dispatches pointer input for both Add and provider cards, ignores preexisting host IDs, verifies exact-provider binding, a durable configuration-only ID-specific `RemoteViews` marker, `LauncherAppWidgetHostView` insertion, matching model/layout spans, cleared pending state, exactly one persisted favorites row, and view/model/database/host-ID cleanup. Fixture components do not enter the release APK.
- Review hardening adds `APPWIDGET_UPDATE` package visibility so receiver-only third-party providers remain enumerable on modern Android. Inaccessible provider configuration activities now catch both `ActivityNotFoundException` and `SecurityException`, log provider context, release the allocated host ID, and clear pending/waiting state. Test cleanup cancels deferred placement and removes its listener before deleting new host IDs, while failed post-grant bind-authority verification rolls back the test-added grant. TDD captured missing-query, denied-configuration process-crash, configured-marker, and pending-placement-cleanup RED states before those paths passed.
- API 35 GREEN passed all 11 `WidgetPickerInstrumentationTest` cases, the external-provider insertion E2E, the pending-placement cleanup instrumentation test, all 7 widget-resize cases, both launcher-menu cases, plus Launcher inflation, drawer open/close, and Preferences reachability smoke tests. App-widget host state matched before and after E2E cleanup. Filtered logcat showed no matching launcher fatal exception, launcher ANR, verifier/missing API failure, `UnsupportedOperationException`, or app-widget service failure.
...
- Final static validation passed 209 JVM tests, Android-test assembly, lint, Docker-wrapper debug APK build, and `git diff --check`. Debug APK: 727,214 bytes; SHA-256 `400831627a9a169f16b77af807f01fd8493870a546b419f452c2993ef7b49eef`.
